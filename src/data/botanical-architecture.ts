...
export const resolveBotanicalArchitecture = (preset: FlowerPreset): BotanicalArchitecture => {
  ...
  return {
    ...
    floweringShootCount: Math.round(clamp(finite(input.floweringShootCount, FALLBACK_ARCHITECTURE.floweringShootCount), 1, 12)),
    basalShootSpreadDeg: clamp(finite(input.basalShootSpreadDeg, FALLBACK_ARCHITECTURE.basalShootSpreadDeg), 0, 42),
